...
const changeStatusByUserId = async (req, res) => {
    try {
        ...
        const reactivatedBodyMail = path.join(config.MAIL_ROUTE, "ractivatedMail.html");
        const reactivatedReadMail = fs.readFileSync(reactivatedBodyMail, "utf-8")
        if(user.status === "banned") {
            sendMail(config.MAIL_SNKRS, "SNKRS", user.email, "Account Banned", readMail)
        } 
        if(user.status === "active"){
            sendMail(config.MAIL_SNKRS, "SNKRS", user.email, "Account Reactivated", reactivatedReadMail)
        }
